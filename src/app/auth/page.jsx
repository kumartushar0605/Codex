...
const AuthPage = () => {
  ...
  const { login: adminLogin, signup: adminSignup, isAuthenticated: adminAuthenticated, loading: authLoading } = useAdmin();
  const { login: userLogin, signup: userSignup, isAuthenticated: userAuthenticated, loading: userLoading } = useUser();
  const [isAdminLogin, setIsAdminLogin] = useState(true); // Add this after isAdmin
  // Remove adminConfirmPassword state
  ...
  const handleAdminToggle = () => {
    ...
    setFormData((prev) => ({ ...prev, fullName: '', email: '', regNumber: '', password: '' }));
    // Remove adminConfirmPassword('');
  };
    ...
    try {
      if (isAdmin) {
        if (isAdminLogin) {
          ...
          // Remove confirm password validation in handleSubmit
          // if (formData.password !== adminConfirmPassword) {
      ...
      <div className="w-full max-w-md">
        ...
        <div className="bg-gradient-to-br from-slate-800/60 to-slate-700/40 backdrop-blur-xl border border-slate-600/50 rounded-2xl shadow-2xl p-8">
          ...
          <form onSubmit={handleSubmit} className="space-y-4">
            {/* Admin Mode: Add sign in/sign up toggle */}
            {isAdmin && (
              <div className="flex bg-slate-700/50 rounded-lg p-1 mb-6">
                <button
                  type="button"
            ...
            {/* Admin Form: Show signup fields if !isAdminLogin */}
            ...
            {/* Admin Form: Show login fields if isAdminLogin */}
            {isAdmin && isAdminLogin && (
              <>
                <div>
            ...
            >
              {loading ? (
                ...
                isAdmin ? (isAdminLogin ? 'Sign In as Admin' : 'Register as Admin') : isLogin ? 'Sign In' : 'Create Account'
              )}
            </button>
          </form>
